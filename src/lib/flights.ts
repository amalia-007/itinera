...
// This is an HONEST ESTIMATE, not a live quote. When an Amadeus key is added,
// swap estimateFlight() for a real fare lookup with the same signature.
...
function seasonMultiplier(dest: Destination, month: number): number {
  ...
  // Global travel peaks (summer + winter holidays) push fares up everywhere.
...
/** Months that avoid the destination's peak demand and global holiday spikes. */
